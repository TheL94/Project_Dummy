...
namespace Multitouch.EventSystems.InputModules
{
    ...
    public class Win10TouchInputModule : StandaloneInputModule
    {
        ...
        private MethodInfo _processTouchPress;
        ...
        new private void ProcessTouchPress(PointerEventData pointerEvent, bool pressed, bool released)
        {
            ...
            {
                var baseType = typeof(StandaloneInputModule);
                var paramTypes = new[] {typeof(PointerEventData), typeof(bool), typeof(bool)};
                _processTouchPress = baseType.GetMethod("ProcessTouchPress",
                    BindingFlags.NonPublic | BindingFlags.Instance, null, paramTypes, null);
                if (_processTouchPress == null)
                    return;
            }
            _paramArray[0] = pointerEvent;
            _paramArray[1] = pressed ? _trueObject : _falseObject;      // prevent auto-boxing by using pre-boxed values
            _paramArray[2] = released ? _trueObject : _falseObject;     // prevent auto-boxing by using pre-boxed values
            _processTouchPress.Invoke(this, _paramArray);
            _paramArray[0] = null;                                      // Do not maintain reference to pointerEvent
        }
